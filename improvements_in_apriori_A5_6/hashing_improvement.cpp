/*
Improve Apriori Algorithm using Hash-Based Approach.
*/
#include <iostream>
#include <vector>
#include <string>
...
int hfunc(vector<int> &item,const vector<pair<int,int>> &frequency_count,const int htable_size){
    //store the index to each transaction count
    unordered_map<int,int> index;
    for(int i=0;i<frequency_count.size();i++){
        index[frequency_count[i].first] = i;
    }

    
    //h(x,y) = (order(x) + order(y)*10 )%SIZE
    int sum = 0;
    //item -> 2 itemset
    sum = ( (frequency_count[index[item[0]]].second * 10)%htable_size + frequency_count[index[item[1]]].second % htable_size)%htable_size;
    return sum;
}

//Hashed based canditature set reduction for 2-itemset
vector<vector<int>> reduce_cset_using_hashing(Dataframe &df,vector<vector<int>> &k_itemset, const vector<pair<int,int>> &frequency_count,const int min_support,const int htable_size = 7){
    vector<vector<int>> reduced_cset;
    int h_position;
    int curr_count;

    //hash table
    vector<vector<vector<int>*>> htable(htable_size); //two dimensional array consisting of address of itemset
    //container to store count of elements in hash_table
    vector<int> count(htable_size,0);

    cout<<"position"<<endl;
    //store the items in the hash table
    for(vector<int> &item : k_itemset){
        curr_count = get_itemset_count(df,item);
        if(curr_count<min_support)
            continue;
        h_position = hfunc(item,frequency_count,htable_size);
        count[h_position] += curr_count;
        htable[h_position].push_back(&item);
    }

    //check if the count of buckets of the hash table is >= minimum support
    for(int i=0;i<htable_size;i++){
        if(count[i]>=min_support){
            for(vector<int>* item_addr:htable[i]){
                reduced_cset.push_back(*item_addr);
            }
        }
    }
    return reduced_cset;
...
//Join and Prune Step, with hashing for 2-itemset
vector<vector<int>> frequent_itemset_using_apriori(Dataframe &df,const int min_support){
    ...
    //for 2-itemset use hashing based technique to reduce the candidature set
    if(Lk.size()!=0){
        Ck = Lk;
        Lk.clear();
         for(int i=0;i<Ck.size();i++){
            for(int j=i+1;j<Ck.size();j++){
                Lk.push_back({Ck[i][0],Ck[j][0]});
            }
        }
        //At this point, we have the 2-itemset canditure set
        //Now we use the hash based tecjnique to reduce the size of canditure set
        Lk = reduce_cset_using_hashing(df,Lk,frequency_count,min_support);
...
int main(int argc,char *argv[]){
    ...
    *df = df->unique(); //check for duplicate transactions in a transaction
    ...
    kwargs["min_support"] = 22.0;
    mine_data(*df,kwargs);
    cout<<endl;

    
    cout<<"---TAKING MIN SUPPORT 30%-----"<<endl;
    kwargs["min_support"] = 30.0;
    mine_data(*df,kwargs);
    cout<<endl;
